...
#define SmokestackAPIVersion    (@"0.2")
#define SmokestackAPIRoot       (@"semop")
#define SmokestackAPIProtocol   (@"http")
...
- (id)init {
    self = [super init];
    if (self) {
        self.pathRoot = [SmokestackAPIRoot stringByAppendingPathComponent:SmokestackAPIVersion];
        self.protocol = SmokestackAPIProtocol;
        self.port = 0;
    }
    return self;
}
...
- (NSString *)authRealm {
    if (!_authRealm) {
        ...
        _authRealm = [NSString stringWithFormat:@"Smokestack/%@/%@/%@", _account, _repo, branch];
    }
    return _authRealm;
}
